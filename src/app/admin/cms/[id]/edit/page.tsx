...
export default function EditPostPage() {
  ...
  useEffect(() => {
    fetch(`/api/admin/posts/${id}`, {
      ...
      .then(r => r.json())
      .then((data: { post?: Post; error?: string }) => {
        if (data.post) {
          const p = data.post;
          setForm({
            title:        p.title,
  ...
  return (
    ...
    <Dialog open={deleteDialog}
      title="Xóa bài viết"
      message={`Xóa bài viết "${form.title}"?\nHành động này không thể hoàn tác.`}
    ...
    <div className="mx-auto max-w-3xl space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-black text-gray-900">Chỉnh sửa bài viết</h1>
          <p className="mt-0.5 font-mono text-[11px] text-gray-400">/{form.slug}</p>
        </div>
      </div>
        ...
        <div className="overflow-hidden rounded-2xl bg-white shadow-[0_1px_12px_rgba(0,0,0,0.06)] ring-1 ring-black/[0.05]">
          <div className="space-y-5 p-6">
            ...
            <div className="flex items-center justify-between rounded-xl bg-gray-50 px-4 py-3">
              <div>
                <p className="text-[13px] font-semibold text-gray-800">Trạng thái đăng bài</p>
                <p className="text-[11px] text-gray-400">Bài viết hiển thị công khai trên website</p>
              </div>
